...
class SingerAlbums extends StatelessWidget {
  ...
  Widget _content(ArtistModel singer, Size size, BuildContext context) {
    return Column(
      children: [
        PlayerAppBar(
            title: "${singer.artistName} - ${S.of(context).albums}",
            leading: IconButton(
                icon: Icon(Icons.arrow_back, color: AppThemeData().iconColor),
                onPressed: () => Navigator.of(context).pop())),
        _singerBackground(context, singer, size),
        Expanded(
            child: FutureBuilder(
                future:
                    AudioCustomQuery().queryAlbums(singer: singer.artistName),
                builder: (scontext, AsyncSnapshot<List<AlbumModel>> snapshot) {
                  ...
                  final List<Widget> widgets = List<Widget>.from(
                      snapshot.data!.map((e) => AlbumTile(album: e)));
  ...
  Widget _singerBackground(
      BuildContext context, ArtistModel singer, Size size) {
    return Container(
      child: Stack(
        children: [
          QueryArtworkWidget(
            ...
            child: Column(
              ...
              children: [
                Row(
                  children: [
                    Text(
                        "${singer.numberOfAlbums} ${S.of(context).albums}\t",
                        style: TextStyle(color: AppThemeData().iconColor)),
                    SizedBox(width: 20.0),
                    Text(
                        "${singer.numberOfTracks} ${S.of(context).tracks}\t",
                        style: TextStyle(color: AppThemeData().iconColor)),
                  ],
                )
              ]
